...
def main(name ):
    ...
    # T_number = [32]
    sigma_number = [2**10,2**9,2 ** 8,2**7, 2 ** 6, 2**5,2 ** 4,2**3]
    # sigma_number = [  2 ,1,2**(-1)]
    # sigma_number = [4,2,1,2**(-1)]
    lambda_number = [0.5]
    # sigma_number = [2**10,2**9,2**8,2**7]
    # lambda_number = [0]
    acc_binary_1 = []
    acc_binary_1_sign = []
    acc_binary_2_linear = []
            ...
            for si in sigma_number:
                ...
                for lamb in lambda_number:
                    ...
                    if method == 2:
                        ...
                        W_fcP = np.asmatrix(np.sign(clf.coef_).T)
                        W_fcP = binary_optimization(x_value, y_temp, W_fcP, class_number, lamb)
                    ...
                    if method == 2:
                        ...
                        acc_binary_1.append(predict_acc(x_value, y_train, test_x, y_test, W_fcP, class_number))
                        #         print(acc_binary_2)
                        non_zeros1.append(np.count_nonzero(W_fcP))
    ...
    if method == 2:
        acc_binary_1 = np.array(acc_binary_1)
        non_zeros1 = np.array(non_zeros1)
        acc_binary_2 = np.array(acc_binary_2)
        non_zeros2 = np.array(non_zeros2)
        acc_binary_2_linear = np.array(acc_binary_2_linear)
        np.save('%s_binary_coefficient' % name, acc_binary_1)
        np.save('%s_non0_binary coefficient' % name, non_zeros1)
